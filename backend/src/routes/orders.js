...
router.post("/", (req, res) => {
  ...
  const requestedPromo =
    req.body && req.body.promo_code ? String(req.body.promo_code) : "";
  if (requestedPromo) {
    const promo = promoCodes.find(
      (candidate) =>
        candidate.code.toLowerCase() === requestedPromo.toLowerCase(),
    );
    const isValid =
      promo &&
      promo.is_active &&
  ...
  const items = userCart.map((cartItem) => {
    const product = products.find(
      (candidate) => candidate.id === cartItem.product_id,
    );
